...
export default function kharidApis({ getState, token, getDateAndTime, showAlert, AIOServiceShowAlert, baseUrl }) {
    return {
        ...
        async getCampaignProducts(campaign) {
            ...
            let res = await this.getProductsByTaxonId({ Taxons: id,cartId:'کمپین' });
        ...
        async newOrders() {
            const taxonProductsList = await this.getProductsByTaxonId({ Taxons: '10932',cartId:'خرید عادی' });
            return getState().updateProductPrice(taxonProductsList);
        },
        async recommendeds() {
            let res = await this.getProductsByTaxonId({ Taxons: '10550',cartId:'خرید عادی' });
            return getState().updateProductPrice(res);
        },
        async bestSellings() {
            let res = await this.getProductsByTaxonId({ Taxons: '10820',cartId:'خرید عادی' });
            return getState().updateProductPrice(res,);
        },
        async preOrders() {
            let { userInfo } = getState();
        ...
        async getCategoryItems(category) {
            ...
            let items = await this.getProductsByTaxonId({ Taxons: category.id.toString(),cartId:'خرید عادی' });
            return getState().updateProductPrice(items, 'kharidApis => getCategoryItems')
        },
        async families() {
            return [
        ...
        getMappedAllProducts({ spreeResult, b1Result, loadType,cartId }) {
            if(!cartId){alert('misiing cartId')}
            if (loadType === 0) {
            ...
            for (let product of products) {
                ...
                finalResult.push({
                    ...
                    price, discountPrice, discountPercent, id: product.id,cartId
                })
            }
            return finalResult;
        },
        ...
        async getTaxonProducts({ loadType, Taxons, Name, cartId }) {
            if(!cartId){alert('missing cartId')}
            let { userInfo } = getState();
            let res = await Axios.post(`${baseUrl}/Spree/Products`,
                {
                    CardCode: userInfo.cardCode,
            ...
            return this.getMappedAllProducts({ spreeResult: spreeData, b1Result: b1Data, loadType,cartId });
        },
        async getProductsByTaxonId({ Taxons,cartId }) {
            let { userInfo } = getState();
            let res = await Axios.post(`${baseUrl}/Spree/Products`,
                {
                    CardCode: userInfo.cardCode,
            ...
            return this.getModifiedProducts({ spreeResult: spreeData, b1Result: b1Data,cartId });
        },
        getModifiedProducts({ spreeResult, b1Result,cartId }) {
                        ...
                        allProducts.push(
                            {
                                ...
                                price: 0, discountPrice: 0, discountPercent: 0, id: product.id,cartId
                            }
                        );
                    }
                }
