...
export default function kharidApis({getState,token,getDateAndTime,showAlert,AIOServiceShowAlert,baseUrl}) {
  return {
    updateProductPrice({products,campaign,cartId}){
        ...
        let config = products.map(({defaultVariant})=>{
            let {code} = defaultVariant;
    ...
    async getCampaigns() {
      let res = await Axios.get(`${baseUrl}/Spree/GetAllCampaigns?ids=10677,10676`);
      ...
      let campaigns = dataResult.map((o) => {
        ...
        else if(o.id === '10677'){icon = eydane_batri}//عیدانه باتری
    ...
    async newOrders() {
      let products=await this.getProductsByTaxonId({Taxons:'10715'});
      return this.updateProductPrice({products,cartId:'خرید عادی'});
    },
    async recommendeds() {
      let products = await this.getProductsByTaxonId({Taxons:'10714'});
      return this.updateProductPrice({products,cartId:'خرید عادی'});
    },
    async bestSellings(){
      let products = await this.getProductsByTaxonId({Taxons:'10709'});
      return this.updateProductPrice({products,cartId:'خرید عادی'});
    },
    async preOrders() {
      let {userInfo} = getState();
