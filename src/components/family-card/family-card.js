        ...
        return (
            <RVD
                layout={{
                    ...
                    onClick:async ()=>{
                        kharidApis({
                            api:'familyProducts',parameter:{id},cache:12 * 60 * 60 * 1000,cacheName:`products-of-family-with-id-${id}`,
                            name:'دریافت خانواده محصولات',
                            callback:(products)=>{
                                openPopup('category',{name:'خانواده ی ' + title,category:{name:'خانواده ی ' + title,products}})
                            }
                        })    
                    },
                    column:[
