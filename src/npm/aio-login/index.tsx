...
export type I_AL_props = {
  ...
  checkToken: (
    token: string | false,
    obj: { userId: string; userInfo?: any },
...
export default class AIOlogin {
  ...
  constructor(props: I_AL_props) {
    ...
    this.setToken = (token: string) =>
      storage.save({ name: "token", value: token });
    this.removeToken = () => storage.save({ name: "token", value: false });
    this.setUserInfo = (userInfo: any, key?: string) =>
      storage.save({
        name: `userInfo${key ? "." + key : ""}`,
        value: userInfo,
      });
    this.getUserInfo = (key?: string) =>
      storage.load({ name: `userInfo${key ? "." + key : ""}` });
    ...
    this.removeStorage = (key: I_AL_storageKey) =>
    ...
    this.render = (p?: I_AL_render_parameter) => {
      ...
      if (profile) {
        if (
          ...
          typeof profile.onSubmit !== "function"
        ) {
          let error = `
      ...
      this.getActions = (p: { setMode: (mode: I_AL_mode) => void }) => {
        if (!this.setMode) {
...
function AIOLOGIN(props: I_AL_AIOLOGIN) {
  ...
  if (splash && showSplash) {
    setTimeout(() => setShowSplash(false), splash.time);
  }
  ...
  function renderSplash() {
    return splash ? splash.render() : null;
  }
  function renderReload() {
    return (
      <div className="aio-login-reload">
  ...
  async function getCheckTokenResult(
    token,
    userId,
    userInfo,
    ...
    let error: string;
    if (!props.checkToken) {
      error = `aio-login error => checkToken props should be a function !!! but is ${typeof props.checkToken}`;
    } else {
      try {
  ...
  function alertMissedToken() {
    new AIOPopup().addAlert({
      ...
      subtext:
        "because register.onSubmitRegister returned true (true means call instance.rendeeApp())",
    });
    setShowReload(true);
  }
  async function onSubmit(model: I_AL_model) {
    try {
      ...
      if (typeof res !== "boolean") {
        new AIOPopup().addAlert({
          type: "error",
          text: "aio login developer warning",
          subtext: "onSubmit should return a boolean",
        });
      }
      if (res === false) {
        setLoading(false);
      ...
      //اگر بررسی رمز عبور موفقیت آمیز است برو برای تصمیم گیری ورود به اپ یا ورود به ثبت نام
        ...
        await setAuthenticated(model.login.userId);
      }
    } catch {
      setLoading(false);
    }
  ...
  async function isRegistered() {
    ...
    if (register && register.checkIsRegistered) {
      ...
      try {
        isRegistered = await register.checkIsRegistered({
          token,
          userId,
          userInfo,
        });
  ...
  async function setAuthenticated(userId?: string) {
    //اگر با کش وارد نشده ایم یوزر آی دی را در کش ثبت کن
    if (userId) {
      setStorage("userId", userId);
    }
    let token = getStorage("token");
    let registered = await isRegistered();
    if (registered === false && register.registerType === "auto") {
      setLoading(false);
      setMode("register");
    } else {
      if (token) {
  ...
  async function onSubmitProfile(profileModel) {
    ...
    setLoading(true);
    try {
      res = await profile.onSubmit(profileModel);
    } catch {
      setLoading(false);
      return;
    }
  ...
  function renderLogin() {
    ...
    if (props.renderLogin) {
      return props.renderLogin(content);
    } else {
      return content;
    }
  }
  if (profile) {
    let props: I_LoginForm = {
      timer,
      id,
      attrs,
      userId,
...
type I_LoginForm = {
  ...
  forget?: I_AL_forget;
  setMode?: (mode: I_AL_mode) => void;
  onSubmit?: (model: I_AL_model) => Promise<void>;
  modes?: I_AL_mode[];
  attrs?: any;
  logout: () => void;
  onSubmitProfile?: (profileModel: any) => void;
  otpLength?: number;
  loading: boolean;
  getStorage: I_AL_getStorage;
  onRegister: (model: I_AL_model) => void;
};
function LoginForm(props: I_LoginForm) {
  ...
  function getLabels(mode: I_AL_mode) {
    ...
    if (mode === "OTPNumber") {
      let subtitle =
        "شماره همراه خود را وارد کنید . پیامکی حاوی کد برای شما ارسال خواهد شد";
      return {
    ...
    if (mode === "register") {
      return {
        inputLabel: false,
        title: registerText("form title"),
        submitText: registerText("submit button"),
        subtitle: false,
  ...
  function title_layout(labels) {
    ...
    return {
      ...
      row: [
        {
          show: !!backButton,
          html: <Icon path={mdiChevronRight} size={1} />,
          size: 48,
          align: "vh",
          onClick: () => {
            if (profile) {
  ...
  function getInput_phoneNumber(field, getValue) {
    return {
      ...
      input: {
        type: "text",
        justNumber: true,
        before: <Icon path={mdiCellphone} size={0.8} />,
        placeholder: "09...",
        maxLength: 11,
  ...
  function getInput_email(field, getValue) {
    return {
      ...
      input: {
        type: "text",
        disabled: !!props.userId,
        before: <Icon path={mdiEmail} size={0.8} />,
      },
      validations: [
        [
          "function",
      ...
      validations: [
        [
          ...
          () => {
            ...
            return value.length !== otpLength
              ? `رمز یکبار مصرف باید شامل ${otpLength} کاراکتر باشد`
              : false;
          },
        ],
      ],
  ...
  function getInput_password(field, type) {
    ...
    return {
      ...
      input: {
        type: "password",
        before: <Icon path={mdiLock} size={0.8} />,
        style: { direction: "ltr" },
        visible: true,
      },
    };
  ...
  function getInputs() {
    ...
    if (mode === "OTPNumber") {
      return [
        getInput_phoneNumber(`value.login.userId`, () => model.login.userId),
      ];
    }
    if (mode === "OTPCode") {
      return [getInput_otp("value.login.password", () => model.login.password)];
    }
    let userIdInput;
    if (mode === "userName") {
      userIdInput = getInput_userName("value.login.userId");
    } else if (mode === "phoneNumber") {
      userIdInput = getInput_phoneNumber(
        "value.login.userId",
        () => model.login.userId,
      );
    } else if (mode === "email") {
      userIdInput = getInput_email(
        "value.login.userId",
        () => model.login.userId,
      );
    }
    ...
    return {
      ...
      html: (
        <AIOInput
          ...
          footer={({ disabled }) =>
            submit_layout({ submitText: labels.submitText, disabled })
          }
        />
      ),
    };
  ...
  async function submit() {
    ...
    if (profile) {
      await onSubmitProfile(model);
    } else if (mode === "register") {
      await onRegister(model);
    } else {
  ...
  function recode_layout() {
    ...
    return {
      ...
      onClick: () => {
        setMode("OTPNumber");
        let newModel: I_AL_model = {
          ...model,
          login: { ...model.login, password: "" },
        };
        setModel(newModel);
      },
    };
      ...
      column: [
        {
          ...
          row: [
            {
              flex: 1,
              html: <div className="aio-login-splitter"></div>,
              align: "v",
            },
      ...
      html: (
        <AIOInput
          ...
          options={[
            ...
            { text: registerText("tab button"), value: "register" },
          ]}
          onChange={(tab) => {
            if (tab === "login") {
              changeMode(modes[0]);
  ...
  function forget_layout() {
    ...
    return {
      ...
      html: buttonText,
      onClick: () => changeMode("forgetUserId"),
    };
  }
...
function SubmitButton(props: I_SubmitButton) {
  ...
  function setLastTry() {
    AIOStorage("aiologinlasttrypermode").save({
      name: "dic",
      value: { ...getLastTry(), [mode]: new Date().getTime() },
    });
    let delta = getDelta();
    setTime(delta);
  }
  ...
  if (time > 0) {
    setTimeout(() => setTime(time - 1), 10000);
  } else if (time < 0) {
  ...
  if (time) {
    ...
    if (!loading) {
      ...
      text = `لطفا چند ثانیه صبر کنید`;
    }
  }
  return (
    <button
      className={
        "aio-login-submit" + (outline ? " aio-login-submit-outline" : "")
      }
      disabled={disabled}
      onClick={() => click()}
    >
