...
import {
  ...
  I_cartShop_Product,
  I_cartShop_bundle,
  I_cartShop_bundle_product,
  I_cartShop_bundle_taxon,
  I_cartShop_taxon,
  I_cartTaxon,
  I_cartVariant,
  I_discount,
  I_product,
  I_product_optionType,
  I_shippingOptions,
  I_renderIn,
  I_variant,
  I_variant_optionValues,
  I_cartProduct,
  I_getFactorDetails_result,
  I_factorItem,
  I_cartShop_bundle_variant,
  I_marketingLine_bundle,
  I_cartShop,
  I_ShopProps,
  I_spreeCategory,
} from "./types";
...
export default class ShopClass implements I_ShopClass {
  ...
  dicToArray = (dic: { [key: string]: any }) =>
    Object.keys(dic).map((key) => dic[key]);
  getTaxonNameByTaxonId = (taxonId: string) => {
    ...
    if (this.itemType === "Taxon") {
      return this.taxons.find((o) => o.id === taxonId).name;
    }
  };
  isProductInCart = (productId: string, taxonId?: string) => {
    let { actionClass } = this.getAppState();
    if (!actionClass.getCartShop(this.shopId)) {
      return false;
    }
    if (taxonId) {
  ...
  getShopItems = async (p?: { taxonId?: string; productId?: string }) => {
    ...
    if (this.itemType === "Bundle") {
      ...
      if (!this.items[taxonId]) {
        let parameter: I_getTaxonProducts_p = {
          category: {
            shopId: this.shopId,
        ...
        let products = await apis.request({
          api: "kharid.getTaxonProducts",
          description: `دریافت محصولات تکزون ${taxonName}`,
          def: [],
          parameter,
          cache: {
            time: 30 * 24 * 60 * 60 * 1000,
            name: `taxonProducts.${this.shopId}.${taxonId}`,
          },
        });
        this.items[taxonId] = products;
      }
      if (!p || !p.productId) {
        return this.items[taxonId];
      }
      let product: I_product = this.items[taxonId].find(
        (o: I_product) => o.id === p.productId,
      );
      return [product];
    } else {
      if (!this.items) {
        let parameter = {
          category: { shopId: this.shopId, shopName: this.shopName },
        };
      ...
      let product = (this.items as I_product[]).find(
        (o: I_product) => o.id === p.productId,
      );
      return [product];
    }
  ...
  openCategory = async (taxonId?: string) => {
    ...
    let props: I_CategoryView = {
      ...
      renderItem: (
        item: I_product | I_bundle_taxon | I_taxon,
        index?: number,
      ) => this.renderCard({ item, renderIn: "category", index }),
    };
    try {
      msfReport({
  ...
  renderCard = (p: {
    item: I_product | I_taxon | I_bundle_taxon;
    renderIn: I_renderIn;
    index?: number;
  }) => {
    let { item, renderIn, index } = p;
    if (this.itemType === "Bundle") {
      return this.renderCard_Bundle({
        taxon: item as I_bundle_taxon,
        renderIn,
        index,
      });
    }
    if (this.itemType === "Taxon") {
      return this.renderCard_taxon({ taxon: item as I_taxon, renderIn });
    }
    if (this.itemType === "Product" || this.itemType === "Category") {
      return this.renderCard_Regular({
        product: item as I_product,
        renderIn,
        index,
      });
    }
  };
  renderCard_Regular = (p: {
    product: I_product;
    renderIn: I_renderIn;
    index: number;
    loading?: boolean;
    ...
    let props: I_RegularCard = {
      ...
      shopId: this.shopId,
      onClick: async () => {
        let newProduct: I_product = await this.openProductPage(
          product,
          "renderCard_Regular",
        );
        return newProduct;
  ...
  renderCard_taxon = (p: { taxon: I_taxon; renderIn: I_renderIn }) => {
    ...
    let props: I_TaxonCard = {
      ...
      renderCard: (product: I_product, index: number) =>
        this.renderCard_Regular({ product, index, renderIn }),
    };
    return <TaxonCard key={taxon.id} {...props} />;
  };
  renderCard_Bundle = (p: {
    taxon: I_bundle_taxon;
    renderIn: I_renderIn;
    index?: number;
  }) => {
    ...
    let props = {
      ...
      renderIn,
      index,
      onClick: () => this.openBundlePage({ taxon }),
    };
  ...
  openBundlePage = (p: { taxon: I_bundle_taxon }) => {
    ...
    msfReport({
      actionName: "open product",
      actionId: 5,
      targetName: taxon.name,
      targetId: taxon.id,
      tagName: "kharid",
      eventName: "page view",
    });
  ...
  updateProduct = (product: I_product) => {
    ...
    if (list) {
      let newList = list.map((o: I_product) =>
        o.id === product.id ? product : o,
      );
  ...
  openProductPage = async (product: I_product, caller: string) => {
    ...
    rsa.addModal({
      ...
      header: {
        title: this.shopName,
        buttons: await actionClass.getHeaderIcons({ cart: true }),
      },
    });
    return product;
  };
  pardakht = async (obj: I_shippingOptions) => {
    //debugger;
    ...
    if (typeof result === "object") {
      ...
      actionClass.openPopup("sefaresh-pardakht-shode", { docNum });
      return true;
    } else {
      return false;
    }
  ...
  payment = async (obj: I_shippingOptions) => {
    //obj => { address, SettleType, PaymentTime, DeliveryType, PayDueDate }
    let { rsa, actionClass } = this.getAppState();
    let result = await this.sabtVizitor(obj);
    if (typeof result === "object") {
      let { orderNumber } = result;
      rsa.removeModal("all");
      actionClass.removeCartTab(this.shopId);
      actionClass.openPopup("sefareshe-ersal-shode-baraye-vizitor", {
        orderNumber,
      });
      return true;
    } else {
  ...
  getCartVariants = async (p?: { productId?: string; taxonId?: string }) => {
    ...
    if (
      ...
      for (let productId in cartTab.products) {
        ...
        if (p && p.taxonId && p.taxonId !== productCategory.taxonId) {
          continue;
        }
        if (p && p.productId && p.productId !== productId) {
          continue;
        }
        let products: I_product[] = (await this.getShopItems({
          taxonId: cartProduct.productCategory.taxonId,
          productId,
        })) as I_product[];
        let product = products[0];
        product = await this.updateProductByFullDetail(
          product,
          "getCartVariant itemType === Product || Category",
        );
        for (let variantId in cartProduct.variants) {
          let { count } = cartProduct.variants[variantId];
          let variant = product.variants.find(
      ...
      let cartTab: I_cartShop_bundle = actionClass.getCartShop(
        "Bundle",
      ) as I_cartShop_bundle;
      for (let taxonId in cartTab.taxons) {
        ...
        for (let productId in products) {
          ...
          for (let variantId in variants) {
            ...
            res.bundleMarketingLines.push({
              ItemCode: variantId,
              ItemQty: count,
              Price: productPrice,
              BasePackCode: taxonId,
              BasePackQty: taxonCount,
            });
  ...
  getAmounts = async (
    shippingOptions: I_shippingOptions,
    container?: string,
  ) => {
    if (this.shopId === "Bundle") {
      return this.getAmounts_Bundle(shippingOptions, container);
    } else {
      return await this.getAmounts_all(shippingOptions, container);
    }
  };

  // ijade section takhfifat
  //data az DiscountList migire
  getAmounts_all = async (
    shippingOptions: I_shippingOptions,
    container?: string,
  ) => {
    //debugger;
    ...
    let factorDetails: I_getFactorDetails_result = actionClass.getFactorDetails(
      marketingLines,
      { ...shippingOptions, CampaignId: this.CampaignId },
      container,
    );
    ...
    if (ShowTotalBfDis && ShowDisValue) {
      discounts.push({
        value: ShowTotalBfDis - ShowDisValue,
        title: "مجموع پس از تخفیف",
      });
    }
    if (PaymentDiscountPercent && PaymentDiscountValue) {
      discounts.push({
        percent: PaymentDiscountPercent.toFixed(2),
        //percent: PaymentDiscountPercent,
        value: PaymentDiscountValue,
        title: "تخفیف نحوه پرداخت",
      });
    }
    if (DiscountValueUsed && DiscountPercentage) {
      discounts.push({
        percent: DiscountPercentage,
        value: DiscountValueUsed,
        title: "کد تخفیف",
      });
    }
    ...
    return {
      total,
      discounts,
      payment: DocumentTotal,
      ClubPoints,
      CampaignId: this.CampaignId,
    };
  };
  getAmounts_Bundle = async (
    shippingOptions: I_shippingOptions,
    container?: string,
  ) => {
    ...
    let discounts: I_discount[] = [];
    if (PayDueDate) {
      let PayDueDateOption = PayDueDate_options.find(
        (o) => o.value === PayDueDate,
      );
      let percent = PayDueDateOption.discountPercent;
      let value = (total * percent) / 100;
      let title = "تخفیف نحوه پرداخت";
      discounts.push({ percent, value, title });
      cashPercent = PayDueDateOption.cashPercent;
      payment -= value;
    }
    let DiscountList =
      actionClass.getCodeDetails({ discountCodeInfo, giftCodeInfo }) || {};
  ...
  renderCartItems = async (renderIn: I_renderIn) => {
    ...
    if (this.itemType === "Bundle") {
      ...
      let cartTaxons: I_cartShop_bundle_taxon[] = this.dicToArray(
        cartTab.taxons,
      );
      ...
      for (let i = 0; i < cartTaxons.length; i++) {
        ...
        let taxon = await shopItems.find(
          (o: I_bundle_taxon) => o.id === taxonId,
        );
      ...
      if (this.itemType === "Category" || this.shopId === "Regular") {
        ...
        let cartTab = actionClass.getCartShop(
          this.shopId,
        ) as I_cartShop_Product;
      ...
      for (let i = 0; i < cartProducts.length; i++) {
        ...
        let products = await this.getShopItems({
          productId,
          taxonId: productCategory.taxonId,
        });
        let product = products[0];
        product = await this.updateProductByFullDetail(
          product,
  ...
  getFactorItems = async (shippingOptions: I_shippingOptions, container) => {
    ...
    if (!total) {
      alert("missing total in ShopClass.getFactorItems");
    }
    if (!payment) {
      alert("missing payment in ShopClass.getFactorItems");
    }
    if (!ClubPoints) {
      alert("missing ClubPoints in ShopClass.getFactorItems");
    }
    let res = [];
    ...
    for (let i = 0; i < discounts.length; i++) {
      ...
      if (!title) {
        alert("missing discount.title in ShopClass.getFactorItems");
      }
      if (!value) {
        alert("missing discount.value in ShopClass.getFactorItems");
      }
    ...
    res.push({
      ...
      value: SplitNumber(this.fix(payment)) + " ریال",
  ...
  edameye_farayande_kharid = () => {
    let { rsa, msfReport } = this.getAppState();
    msfReport({
      actionName: "open checkout page",
      actionId: 754,
      targetName: this.shopName,
      targetId: this.shopId,
      tagName: "kharid",
      eventName: "page view",
    });
    rsa.addModal({
      id: "edameye_farayande_kharid",
      position: "fullscreen",
      body: {
        render: () => (
          <Shipping shopId={this.shopId} shopName={this.shopName} />
        ),
      },
      header: { title: "ادامه فرایند خرید" },
    });
  };
  sabtVizitor = async (obj) => {
    // obj => { address, SettleType, PaymentTime, DeliveryType, PayDueDate }
    ...
    if (res.data.isSuccess) {
      try {
        return { orderNumber: res.data.data[0].docNum };
      } 
      catch {
      ...
      rsa.addAlert({ text: res.data.message, type: "error" });
      return false;
    }
  };
  sabt = async (obj) => {
    ...
    let res = await Axios.post(`${baseUrl}/BOne/AddNewOrder`, body);
    if (!res.data.isSuccess) {
        rsa.addAlert({ text: res.data.message, type: "warning" });
        return false;
    } 
    //console.log(res.data.message);
    let registredOrder;
    try {
      registredOrder = res.data.data[0];
    } 
    catch {
    ...
    let result = await Axios.post(`${baseUrl}/payment/request`, {
      ...
      CallbackUrl:
        baseUrl === "https://uiretailerapp.bbeta.ir/"
          ? "https://uiretailerapp.bbeta.ir/"
          : "https://bazar.miarze.com/",
    });
  ...
  getOrderBody = async ({
    ...
    let DiscountList = actionClass.getCodeDetails({
      giftCodeInfo,
      discountCodeInfo,
    });
    let marketingLines;
  ...
  getPaymentButtonText = (shippingOptions: I_shippingOptions) => {
    //if (this.shopId === 'Bundle')
    //{ return shippingOptions.SettleType !== 2 ? 'پرداخت' : 'ثبت' }
    // else { return 'ارسال برای ویزیتور' }

    return shippingOptions.PaymentTime === 5 ? "پرداخت" : "ارسال برای ویزیتور";
  };
  hasCartError = () => {
    let { actionClass } = this.getAppState();
    if (this.itemType !== "Taxon") {
  ...
  //                 column:[
  //                     {
  //                         //show: payment > maxTotal ,style:{color:'orange'},
  //                         show:!!description,style:{color:'orange'},
  //                         // row:[
  //                         //     {html:<Icon path={mdiInformation} size={0.8}/>,align:'vh',size:36},
  //                         //     {
  //                         //         html:`حداکثر مبلغ مجاز این فاکتور ${SplitNumber(this.maxTotal)} ریال است`,align:'v',className:'fs-12'
  //                         //     }
  //                         // ]
  //                     },
  //                     {
  ...
  //                                 html: () => (
  //                                     <button
  //                                         disabled={payment === 0 || payment > maxTotal ? true : false}
  //                                         onClick={() => {
  //                                             if(hasError){
  ...
  //     )
  // }
  renderCartFactor = async (button?: boolean,container?: string,)=> {
    //debugger
    let campaignInformation = "";
    let { actionClass } = this.getAppState();
    let res = await this.getAmounts(undefined, "cart");
    let { payment = 0, total } = res;
    //console.log("RES:", res);
    let disabled = false;
    let description = "";
    if (payment === 0) {
    ...
    if (this.CampaignId === 63) {
      let { marketingLines } = await this.getCartVariants();
      let factorDetails: I_getFactorDetails_result = actionClass.getFactorDetails(
        marketingLines,
        { CampaignId: this.CampaignId },
        container,
      ...
      let allItemQtyZero = true; // متغیر برای بررسی صفر بودن تمام ItemQty ها
    
      // استفاده از forEach برای اطمینان از پردازش تمام آیتم‌ها
      MarketingLines.forEach(line => {
        let { CampaignDetails = {}, ItemCode, ItemQty } = line;
        let { Information, BundleRowsInfos, RequestedQty, Status } = CampaignDetails;
        let { minQty, maxQty, qty } = BundleRowsInfos; 
        if (ItemQty > 0) {
          allItemQtyZero = false; // اگر یک ItemQty بیشتر از صفر بود، این متغیر را false می‌کنیم
        }
    
        if (RequestedQty > maxQty || RequestedQty < minQty) {
          campaignInformation = Information;
          // متوقف کردن حلقه در صورت عدم تطابق با محدوده درخواست شده
          return;
        }
      });
      ...
      if (allItemQtyZero) {
        ...
        disabled = false; // اگر حتی یک ItemQty بیشتر از صفر باشد، disabled به false تنظیم می‌شود
      }
    }
    ...
    return (
      <RVD
        layout={{
          ...
          column: [
            {
              show: !!description && !campaignInformation,
              style: { color: "red" },
              row: [
                {
                  html: <Icon path={mdiInformation} size={0.8} />,
                  align: "h",
                  size: 36
                },
                {
                  ...
                  column: [
                    ...
                    },
                    {
                      row: [
                        {
                          align: "v",
                          html: SplitNumber(payment),
                          className: "theme-dark-font-color fs-20 bold",
                        },
                        { size: 4 },
                        {
                          align: "v",
                          html: " ریال",
                          className: "theme-dark-font-color fs-12",
                        },
                      ],
                    },
                    { flex: 1 },
                ...
                },
                {
                  html: () => (
                    <button
                      ...
                      className={hasError ? "button-3" : "button-2"}
                      style={{ height: 36, padding: "0 12px" }}
                    >
                      {hasError ? "اصلاح سبد خرید" : "ادامه فرایند خرید"}
                    </button>
                  ),
                  align: "v",
                },
...
type I_CategoryView = {
  ...
  description?: string;
  renderItem: Function;
  items: (I_product | I_bundle_taxon | I_taxon | I_spreeCategory)[];
...
function CategoryView(props: I_CategoryView) {
  ...
  function description_layout() {
    ...
    return {
      html: <pre className="category-view-description">{description}</pre>,
  ...
  function tabs_layout() {
    ...
    let options = categories.map((category) => {
      ...
      return {
        text: <img src={icon} width="60" height="60" />,
        value: id,
        subtext: name,
      };
    });
    return {
      html: (
        <AIOInput
          ...
          options={options}
          value={tab}
          onChange={(tab: string) => changeTab(tab)}
        />
      ),
    };
  }
  function products_layout() {
    return {
      //gap: 12,
      column: getProductsBySearch().map((product, index) =>
        product_layout(product, index),
      ),
    };
  }
  function product_layout(product, index) {
    if (searchValue && product.name.indexOf(searchValue) === -1) {
      return false;
    }
    let html = renderItem(product, index);
    return { html, className: "of-visible hide-scroll" };
  }
  function getProductsBySearch() {
    ...
    return items.filter(
      (o) => !searchValue || o.name.indexOf(searchValue) !== -1,
    );
  }
  function body_layout() {
    return {
      flex: 1,
      className: "ofy-auto hide-scroll m-b-12",
      column: [banner_layout(), description_layout(), products_layout()],
    };
  }
...
function TaxonCard(props: I_TaxonCard) {
  ...
  let [open, setOpen] = useState<boolean>(
    storage.load({ name: "toggle" + taxon.id, def: false }),
  );
  ...
  async function onClick(e) {
    e.stopPropagation();
    setOpen(!open);
    storage.save({ name: "toggle" + taxon.id, value: !open });
  }
  function name_layout() {
    return { html: taxon.name, flex: 1, align: "v" };
  }
  function icon_layout() {
    return {
      size: 36,
      align: "vh",
      html: <Icon path={open ? mdiChevronDown : mdiChevronLeft} size={0.8} />,
    };
  }
  function groupDiscount_layout() {
    ...
    let notHasErrors = taxonIds.filter(
      (taxonId) => !cartTab.taxons[taxonId].hasError,
    );
    let cartTaxon: I_cartTaxon = cartTab.taxons[taxon.id];
    if (!cartTaxon) {
      return false;
    } else {
      if (cartTaxon.hasError) {
        return false;
      }
    }
    return {
      align: "v",
      className: "taxon-card-group-discount",
      gap: 6,
      row: [
        {
          html: `${1 * notHasErrors.length} %`,
  ...
  function header_layout() {
    return {
      className: "taxon-card-header",
      onClick,
      row: [icon_layout(), name_layout()],
    };
  }
  function products_layout() {
    if (!open) {
    ...
    return {
      className: "p-6 of-visible hide-scroll",
      column: products.map((product: I_product, index: number) => {
        if (renderIn === "cart" || renderIn === "shipping") {
          if (!Shop[shopId].isProductInCart(product.id, taxon.id)) {
            return false;
          }
        }
  ...
  function product_layout(product: I_product, index: number) {
    return {
      className: "of-visible hide-scroll",
      html: renderCard(product, index),
    };
  }
      ...
      row: [
        {
          html: (
            <Icon
              path={type === "min" ? mdiArrowDown : mdiArrowUp}
              size={0.7}
            />
          ),
          align: "vh",
        ...
        {
          html: `${type === "min" ? "حداقل" : "حداکثر"} مبلغ ${SplitNumber(taxon[type])} ریال`,
          align: "v",
        },
      ],
    };
  ...
  function range_layout() {
    return {
      align: "v",
      className: "taxon-card-range",
      row: [rangeItem_layout("min"), { flex: 1 }, rangeItem_layout("max")],
    };
  }
  function footer_layout() {
    return {
      className: "taxon-card-footer",
      column: [groupDiscount_layout(), range_layout()],
    };
  }
  return (
    <RVD
      layout={{
        ...
        column: [header_layout(), products_layout()],
...
type I_RegularCard = {
  ...
  container?: string;
  LineConditions?: LineCondition[];
  onClick: (product: I_product) => Promise<I_product>;
  type?: "horizontal" | "vertical";
};
function RegularCard(props: I_RegularCard) {
  let { Shop, cart, actionClass }: I_app_state = useContext(appContext);
  let {
    shopId,
    renderIn,
    index,
  ...
  // let [variantCounts, setVariantCounts] = useState<{ [key: string]: number }>({});
  // let [totalVariantCount, setTotalVariantCount] = useState<number>(0);
  let [itemQty, setItemQty] = useState(0);
  const [itemQtyList, setItemQtyList] = useState<{ [key: string]: number }>({});
  const [maxQtyList, setMaxQtyList] = useState<{ [key: string]: number }>({});
  const [minQtyList, setMinQtyList] = useState<{ [key: string]: number }>({});
  const [minValueList, setMinValueList] = useState<{ [key: string]: number }>({});
  const [infoList, setInfoList] = useState<{ [key: string]: string }>({});
  ...
  function image_layout(size) {
    ...
    return {
      size,
      align: "vh",
      html: <img src={images[0] || (NoSrc as string)} width={"100%"} alt="" />,
    };
  }
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let { marketingLines } = await Shop[shopId].getCartVariants();
          let factorDetails: I_getFactorDetails_result = actionClass.getFactorDetails(
            marketingLines,
            { CampaignId: Shop[shopId].CampaignId }
          );
          let { MarketingLines } = factorDetails;
          ...
          let minQtyTemp = {};
          let minValueTemp = {};
          let infoTemp = {};  // شیء جدید برای ذخیره اطلاعات Information
          console.log(MarketingLines)
          for (let i = 0; i < MarketingLines.length; i++) {
            let { CampaignDetails = {}, ItemCode, ItemQty } = MarketingLines[i];
            let { Information, BundleRowsInfos = {}, RequestedQty, Status, RequestedValue } = CampaignDetails;
            let { minQty, maxQty, qty, maxQtyCanOrder, maxValue, minValue } = BundleRowsInfos;
            
            setItemQty(ItemQty)
  
            itemQtyTemp[ItemCode] = ItemQty;
            maxQtyTemp[ItemCode] = maxQty;
            minQtyTemp[ItemCode] = minQty;
            minValueTemp[ItemCode] = minValue;
            infoTemp[ItemCode] = Information;
          }
  
          setItemQtyList(itemQtyTemp);
          setMaxQtyList(maxQtyTemp);
          setMinQtyList(minQtyTemp);
          setMinValueList(minValueTemp);
          setInfoList(infoTemp);  // ذخیره شیء جدید در state
      } 
      catch (error) {
    ...
    fetchData();
  }, [Shop, shopId, actionClass,reload]);
  ...
  function checkItemQtyAndMaxQty(itemCode) {
    const itemQty = itemQtyList[itemCode] || 0;
    const maxQty = maxQtyList[itemCode] || 0;
    return itemQty;
  }

  function checkItemMinQty(itemCode) {
    const minQty = minQtyList[itemCode] || 0;
    return minQty;
  }

  function checkItemMinValue(itemCode) {
    const minValue = minValueList[itemCode] || 0;
    return minValue;
  }

  function checkItemInfo(itemCode) {
    const info = infoList[itemCode] || '';
    return info;
  }
  ...
  //جدا کننده متن خطا
  function splitTextAfterFirstDot(text) {
  ...
  function variantError_layout(info) {
    const processedInfo = splitTextAfterFirstDot(info);
    
    return {
      //align: 'v',
      className: "regular-card-variant-error align-v",
      row: [
        { html: <Icon path={mdiInformation} size={0.6} />, size: 20}, 
        { html: <div className="info" dangerouslySetInnerHTML={{ __html: processedInfo }} />},
      ],
    }
  }

  function variant_layout(cartVariant: I_cartVariant) {
    let { count, variantId, error, ItemCode } = cartVariant;
    let itemCode = ItemCode || variantId; 
    //let itemCode = cartVariant.ItemCode; // Assuming ItemCode is part of cartVariant
    count = checkItemQtyAndMaxQty(itemCode);
    let info = infoList[itemCode] || '';
          ...
          row: [
            {
              ...
              align: "v",
              column: [
                variantLabels_layout(variantId),
                VariantAmount_layout(itemCode),
              ],
            },
            //countButton_layout(variantId),
            renderIn === "shipping" ? Icon_layout(itemCode) : countButton_layout(variantId),
          ],
        },
        //variantError_layout(info),
        info ? variantError_layout(info) : '',
      ],
    };
  }
  ...
  function VariantAmount_layout(itemCode?:string) {

    itemQty = checkItemQtyAndMaxQty(itemCode);

    // let unitTotal = product.FinalPrice * itemQty;
    // if (isNaN(unitTotal)) {
    //   return false;
    // }  
    
    // Find the specific variant based on the item code
    const variant = product.variants.find(v => v.id === itemCode);

    // Check if the variant exists and has a valid FinalPrice
    if (!variant || isNaN(variant.FinalPrice)) {
        return false;
    }

    // Calculate the unit total using the FinalPrice of the variant
    let unitTotal = variant.FinalPrice * itemQty;

    return {
        align: 'v', gap: 3,
        row: [
          { html: `جمع ${itemQty} عدد`, className: 'theme-light-font-color fs-10' },
  ...
  function Icon_layout(itemCode?:string) {
    itemQty = checkItemQtyAndMaxQty(itemCode);
    return {
      align: "h",
      gap: 3,
      ...
      row: [
        ...
        { html: itemQty, align: 'v', className: 'fs-18' }
      ],
    }
  }
  ...
  function variantLabels_layout(variantId: string) {
    ...
    return {
      align: "v",
      className: "theme-medium-font-color fs-10 bold",
      gap: 6,
      gepHtml: () => "-",
      row: details.map(([key, value]) => {
        return variantLabel_layout(key, value);
      }),
    };
  }
  function variantLabel_layout(key: string, value: string = "") {
    ...
    let before;
    if (value === "زرد") {
      before = <img src={zardsrc as string} width="20" alt="" />;
    } else if (value === "قرمز") {
      before = <img src={ghermezsrc as string} width="20" alt="" />;
    } else if (value === "نارنجی") {
      before = <img src={narenjisrc as string} width="20" alt="" />;
    } else if (value === "سبز") {
      before = <img src={sabzsrc as string} width="20" alt="" />;
    } else if (value === "آبی") {
      before = <img src={abisrc as string} width="20" alt="" />;
    } else if (value === "یخی") {
      before = <img src={yakhisrc as string} width="20" alt="" />;
    }
    return {
      gap: 2,
      align: "v",
      row: [{ show: !!before, html: () => before }, { html: value }],
    };
  }
  //نام کمپین که نارنجی رنگ نوشته شده در کارت محصولات
  function title_layout() {
    return {
      html: Shop[shopId].shopName,
      className: "fs-10",
      style: { color: "rgb(253, 185, 19)" },
    };
  }
  function numInSale_layout() {
    if (product.SalesMeasureUnit === 'شعله') {
      return {
        html: "کف خرید کالا : " + product.NumInSale + " عدد",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
      };
    }
    else if (product.name === 'کلید مینیاتوری 6000( تک پل ) - ساخت ترکیه ') {
        return {
            html: "کف خرید کالا : " + 12 + " عدد",
            className: "fs-12 theme-light-font-color",
            style: { textAlign: "right" },
        };
    }
    else if (product.name === 'کلید مینیاتوری 6000( سه پل ) - ساخت ترکیه ') {
        return {
            html: "کف خرید کالا : " + 4 + " عدد",
            className: "fs-12 theme-light-font-color",
            style: { textAlign: "right" },
        };
    }
    else if (product.name === ' باتری قلمی  بلیستر سوپر آلکالاین دوتایی') {
        return {
            html: "کف خرید کالا : " + 40 + " عدد",
            className: "fs-12 theme-light-font-color",
            style: { textAlign: "right" },
        };
    }
    else if (product.SalesMeasureUnit === 'عدد') {
      return {
        html: "کف خرید کالا : " + product.NumInSale + " عدد",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
    ...
    else if (product.SalesMeasureUnit === "دو عددی") {
        return {
            html: "کف خرید کالا : "  + 60 +  " عدد",
            className: "fs-12 theme-light-font-color",
            style: { textAlign: "right" },
        };
    }
    else if (product.SalesMeasureUnit === "چهار عددی") {
        return {
            html: "کف خرید کالا : " + 40 + " عدد",
            className: "fs-12 theme-light-font-color",
            style: { textAlign: "right" },
    ...
    else if (product.SalesMeasureUnit === "بسته 10 عددی") {
      return {
        html: "کف خرید کالا : " + 5 + " عدد",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
      };
    }
    else if (product.SalesMeasureUnit === "جعبه 50 عددی") {
      return {
        html: "کف خرید کالا : " + 5 + " عدد",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
    ...
    else if (product.SalesMeasureUnit === 'جفت') {
      return {
        html: "کف خرید کالا : 12,200,000 ریال",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
      };
    }
    else if (product.SalesMeasureUnit === 'بسته') {
      return {
        html: "کف خرید کالا : 12,200,000 ریال",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
    ...
    else if (product.SalesMeasureUnit === "رول 50 متری") {
      return {
        html: "کف خرید کالا : " + 1 + " عدد",
        className: "fs-12 theme-light-font-color",
        style: { textAlign: "right" },
      };
    }
  }
  function DeliveryDate_layout() {
    return {
      html: product.AppDelDsc,
      className: "fs-12 theme-light-font-color align-v",
      style: { textAlign: "right" },
    };
  }
  function name_layout(fontSize) {
    return {
      html: product.name,
      className: `fs-${fontSize} theme-medium-font-color bold`,
  ...
  function discount_layout() {
    ...
    return {
      ...
      row: [
        {
          show: !!B1Dscnt || !!CmpgnDscnt || !!PymntDscnt || !!discountPercent,
          html: <del>{SplitNumber(Price)}</del>,
          className: "fs-12 theme-light-font-color",
        },
        dps_layout(),
      ],
  ...
  function dps_layout() {
    ...
    return {
      gap: 3,
      row: [
        dp_layout(B1Dscnt, "#13a73a"),
        camp_layout(CmpgnDscnt, "#13a73a"),
        dp_layout(PymntDscnt, "#f75b00"),
        dp_layout(discountPercent, "#ff4335"),
      ],
    };
  }
  function dp_layout(percent: number, color) {
    if (Shop[shopId].shopName === 'اقلامی 403' || Shop[shopId].shopName === 'ده سالگی بروکس') {
      return !percent
      ? false
      : {
          html: (
            <div className="discount-percent" style={{ background: color }}>
              { (percent + 2 ).toFixed(0) + "%"}
            </div>
          ),
        };
  ...
  function camp_layout(percent: number, color) {
    return !percent
      ? false
      : 
      {
  ...
  function notExist_layout() {
    return {
      row: [
        { flex: 1 },
        { html: "ناموجود", className: "colorD83B01 bold fs-12" },
        { size: 12 },
      ],
    };
  }
  function price_layout() {
    ...
    return {
      row: [
        { size: 6 },
        {
          html: "هر " + product.SalesMeasureUnit,
          className: "theme-light-font-color fs-12",
        },
        {
          html: SplitNumber(FinalPrice),
          className: "fs-12 theme-dark-font-color bold p-h-6",
        },
        { html: " ریال ", className: "theme-light-font-color fs-12 p-l-3" },
      ],
    };
  }
  useEffect(() => {
  ...
  function horizontal_layout() {
    return (
      <RVD
        ...
        {
          className:
            "theme-card-bg theme-box-shadow gap-no-color rvd-rotate-card regular-card" +
            (mounted ? " mounted" : ""),
          onClick: async () => {
              ...
              row: [
                image_layout(116),
                {
                  ...
                  column: [
                    ...
                    title_layout(),
                    name_layout(12),
                    (Shop[shopId].shopName === 'اقلامی 403' || Shop[shopId].shopName === 'ده سالگی بروکس') ? numInSale_layout() : '',
                    //DeliveryDate_layout(),
                    { row: [{align:'h'}, DeliveryDate_layout()] },
                    !product.description ? false : description_layout(),
                    { flex: 1 },
                    { row: [{ flex: 1 }, discount_layout()] },
                    product.inStock ? false : notExist_layout(),
                    product.Price === 0 ? notExist_layout() : false,
              ...
              row: [
                ...
                { size: 4 },
                {
                  ...
                  column: [
                    ...
                    {
                      align: "v",
                      row: [price_layout(), { flex: 1 }, discount_layout()],
                    },
                    product.inStock ? false : notExist_layout(),
                    { size: 6 },
                  ],
  ...
  function vertical_layout() {
    ...
    return (
      <RVD
        ...
        layout={{
          className:
            "theme-card-bg theme-box-shadow theme-border-radius of-visible hide-scroll w-168 h-288 fs-14 br-12",
          onClick,
          column: [
            {
              size: 140,
            ...
            {
              html: name,
              className: "fs-12 p-v-6 p-h-12 theme-medium-font-color bold align-v",
              style: { whiteSpace: "normal" },
            },
            // {
            //   html: DeliveryDate_layout(),
            //   className: "fs-12 p-v-6 p-h-12 theme-medium-font-color bold",
            //   style: { whiteSpace: "normal" },
            // },
            { className:'p-r-12',column: [DeliveryDate_layout()] },
            { flex: 1 },
            { row: [{ flex: 1 }, discount_layout()] },
            { row: [{ flex: 1 }, price_layout()] },
            product.inStock ? false : notExist_layout(),
  ...
  return type === "horizontal" ? ((product.inStock && product.Price > 0) ? horizontal_layout() : '') : vertical_layout();
}
type I_BundleCard = {
  taxon: I_bundle_taxon;
  renderIn: I_renderIn;
...
function BundleCard(props: I_BundleCard) {
    ...
    return {
      row: [
        ...
        { size: 3 },
        {
          ...
          html: (
            <div
              style={{ height: 2, width: "1100%", background: "#FDB913" }}
            ></div>
          ),
          align: "v",
        },
  ...
  function image_layout() {
    return {
      flex: 1,
      html: <img src={taxon.image} alt="" width="100%" className="br-12" />,
    };
  }
  ...
  function name_layout() {
    return {
      html: taxon.name,
      className: "fs-14 theme-dark-font-color bold",
      style: { textAlign: "right" },
    };
  }
  function detail_layout() {
    if (renderIn === "cart") {
      ...
      for (let i = 0; i < taxon.products.length; i++) {
        ...
        let cartProduct: I_cartShop_bundle_product =
          cartTaxon.products[product.id];
        let str = `${product.name} ( `;
        for (let j = 0; j < product.variants.length; j++) {
          let variant = product.variants[j];
          let cartVariant: I_cartShop_bundle_variant =
            cartProduct.variants[variant.id];
      ...
      return {
        html: ps.join("***"),
        className: "fs-12 theme-medium-font-color",
        style: { textAlign: "right" },
      };
    }
    let ps = [];
  ...
  function price_layout() {
    return {
      ...
      row: [
        {
          flex: 1,
          className: "fs-14 bold theme-dark-font-color",
          align: "v",
          row: [{ flex: 1 }, { html: `${SplitNumber(taxon.price)} ریال` }],
        },
      ],
    };
  }
  ...
  return (
    <RVD
      layout={{
        className:
          "theme-box-shadow theme-card-bg theme-border-radius theme-gap-h p-12 of-visible hide-scroll rvd-rotate-card" +
          (mounted ? " mounted" : ""),
        onClick,
        column: [
          title_layout(),
...
function RegularPage(props: I_RegularPage) {
  ...
  let { shopName } = product.category;

  let [selectedVariant, setSelectedVariant] = useState<I_variant | undefined>();
  let [selectedDic, setSelectedDic] = useState<
    I_variant_optionValues | undefined
  >();
  let [existOptionValueNames, setExistOptionValueNames] = useState<string[]>(
    [],
  );
  let [variantOptions, setVariantOptions] = useState<
  ...
  useEffect(() => {
    ...
    let firstVariant: I_variant = product.inStock
      ? product.variants.filter((o) => o.inStock)[0]
      : undefined;
    let selectedDic: I_variant_optionValues = firstVariant
    ...
    pushToDataLayer({     
      event: 'page_view',
      page_title: `خرید محصول ${product.name}`,
      page_url: `/burux_shop/product`,
      ecommerce: {
        items: {
          name: product.name,
          price: product.Price,
          productCategory: product.category.taxonName,
          variants,
          campaign: shopName,
        }
      }
  ...
  function isVariantMatchBySelected(
    variant: I_variant,
    newSelectedDic: I_variant_optionValues,
  ) {
    let { optionValues } = variant;
    for (let optionTypeId in newSelectedDic) {
      if (optionValues[optionTypeId] !== newSelectedDic[optionTypeId]) {
        return false;
      }
    }
    return true;
  ...
  function getVariantBySelected(newSelectedDic: I_variant_optionValues) {
    for (let i = 0; i < variants.length; i++) {
      ...
      if (isVariantMatchBySelected(variant, newSelectedDic)) {
        return variant;
      }
    }
  }
  ...
  function body_layout() {
    ...
    return {
      flex: 1,
      className: "ofy-auto hide-scroll",
      gap: 12,
      column: [
        image_layout(),
        options_layout(),
        optionTypes_layout(optionTypes),
        details.length !== 0 ? details_layout(details) : "",
      ],
    };
  }
  function image_layout() {
    ...
    return {
      size: 346,
      className:
        "theme-box-shadow theme-card-bg theme-border-radius m-h-12 m-t-12",
      column: [
        { size: 24 },
        {
          flex: 1,
          ...
          row: [
            ...
            {
              flex: 1,
              html: <img src={product.images[srcIndex]} alt="" height="100%" />,
            },
              ...
              column: [
                {
                  size: 24,
                  html: product.name,
                  className: "fs-14 theme-dark-font-color bold p-h-8",
                },
                {
                  size: 24,
                  html: () => "تعداد در کارتن : " + product.NumInSale + " عدد",
                  className: "fs-12 theme-medium-font-color p-h-8",
                },
              ],
            },
            { flex: 1 },
              ...
              column: [
                {
                  size: 24,
                  html: product.AppDelDsc,
                  className: "fs-12 theme-medium-font-color p-l-12",
                },
                {
                  show: !!selectedVariant,
                  size: 24,
                  html: () => "کد کالا : " + selectedVariant.id,
                  className: "fs-12 theme-medium-font-color p-l-12",
                },
              ],
            },
  ...
  function options_layout() {
    ...
    return {
      className: "theme-card-bg theme-box-shadow theme-border-radius m-h-12",
      hide_xs: true,
      column: [
        { size: 12 },
        {
          row: [
            {
              html: `(${variantOptions.length}) انتخاب اقلام موجود`,
              align: "v",
              className: "p-h-12 theme-medium-font-color fs-14 bold",
              style: { direction: "ltr" },
            },
            { flex: 1 },
          ],
        },
        {
          ...
          html: (
            <AIOInput
              type="select"
              className="product-exist-options"
              ...
              onChange={(value) =>
                changeOptionType(
                  variants.find((o) => o.id === value).optionValues,
                )
              }
            />
          ),
  ...
  function optionTypes_layout(optionTypes: I_product_optionType[]) {
    ...
    return {
      className:
        "theme-card-bg theme-box-shadow theme-border-radius gap-no-color m-h-12 p-12",
      column: [
        {
          gap: 6,
          column: optionTypes.map((optionType: I_product_optionType, i) => {
            ...
            return {
              column: [
                {
                  show: optionValueIds.length > 0,
                  html: optionType.name,
                  align: "v",
                  className: "fs-12 theme-medium-font-color bold h-24",
                },
                { size: 6 },
                {
                  className: "ofx-auto hide-scroll",
                  gap: 12,
                  row: optionValueIds.map((optionValueId) => {
                    ...
                    let active = value
                      ? value.toString() === optionValueId
                      : false;
                    let disable = active ? optionValueName === "بروکس" : false;
                    let className =
                      "fs-12 p-v-3 p-h-12 br-4 product-option-value" +
                      (active ? " active" : "") +
                      (disable ? " disable" : "");   
                    return {
                      html: optionValueName,
                      align: "vh",
                      className,
                      onClick: () =>
                        changeOptionType({ [optionType.id]: optionValueId }),
                    };
                  }),
                },
                // { show: optionValueIds.length > 1, html: `لطفا ${optionType.name} مورد نظر خود را انتخاب کنید!`, align: "v", className: "fs-10 h-24", style:{color:'red',paddingTop:'5px'} },
              ],
            };
          }),
        },
  ...
  function details_layout(details) {
    return {
      className:
        "theme-card-bg theme-box-shadow theme-border-radius p-12 m-h-12",
      column: [
        {
          size: 36,
          childsProps: { align: "v" },
          ...
          row: [
            // { size: 24, align: 'vh', html: <Icon path={showDetails ? mdiChevronDown : mdiChevronLeft} size={0.8} /> },
            {
              size: 24,
              align: "vh",
              html: <Icon path={mdiChevronDown} size={0.8} />,
            },
        ...
        },
        {
          ...
          html: () => (
            <div
              className="w-100 br-4 of-hidden"
              style={{
                display: "grid",
            ...
            >
              {details.map((o, i) => {
                ...
                let props = {
                  className: "fs-12 theme-medium-font-color p-v-6 p-h-12",
  ...
  function showCart_layout() {
    ...
    return {
      ...
      className: "p-h-12 bgFFF",
      size: 36,
      align: "v",
      row: [
        {
          html: "مشاهده سبد خرید",
          className: "theme-light-font-color fs-12 bold",
          align: "v",
        },
        { size: 4 },
        {
          html: <Icon path={mdiChevronLeft} size={0.8} />,
          align: "vh",
  ...
  function footer_layout() {
    return {
      size: 70,
      style: { boxShadow: "0 0px 6px 1px rgba(0,0,0,.1)" },
      className: "p-h-12 bg-fff",
      row: [addToCart_layout(), { flex: 1 }, price_layout()],
    };
  }
  function addToCart_layout() {
    if (!selectedVariant || !selectedVariant.inStock || ( selectedVariant.Price === 0 )) {
      return { html: "" };
    }
    return {
      column: [
  ...
  function price_layout() {
    if (!selectedVariant || !selectedVariant.inStock || ( selectedVariant.Price === 0 ) ) {
      return {
        column: [
          { flex: 1 },
          { html: "ناموجود", className: "colorD83B01 bold fs-14" },
          { flex: 1 },
        ],
      };
    }
      ...
      column: [
        { flex: 1 },
        {
          row: [
            { flex: 1 },
            {
              show: !!B1Dscnt || !!CmpgnDscnt || !!PymntDscnt,
              html: () => <del>{SplitNumber(selectedVariant.Price)}</del>,
              className: "theme-light-font-color",
            },
            { size: 1 },
            {
              html: () => "%" + B1Dscnt.toFixed(0),
              show: !!B1Dscnt,
              style: {
                background: "#13a73a",
                color: "#fff",
            ...
            { size: 1 },
            {
              html: () => Shop[shopId].shopName === 'اقلامی 403' || Shop[shopId].shopName === 'ده سالگی بروکس' ? ("%" + ((PymntDscnt + 2).toFixed(0))) : ("%" + PymntDscnt.toFixed(0)),
              show: !!PymntDscnt,
              style: {
                background: "#ff4335",
                color: "#fff",
        ...
        },
        {
          row: [
            ...
            { size: 6 },
            {
              html: SplitNumber(selectedVariant.FinalPrice),
              className: "theme-dark-font-color bold",
            },
            { size: 6 },
            { html: "ریال", className: "theme-light-font-color fs-12" },
          ],
        },
        { flex: 1 },
      ],
  ...
  return (
    <RVD
      layout={{
        className: "theme-popup-bg",
        gap: 12,
        column: [body_layout(), showCart_layout(), footer_layout()],
      }}
    />
  );
}
...
function BundlePage(props: I_BundlePage) {
  ...
  let { taxon } = props;
  if (typeof taxon.max !== "number") {
    taxon.max = Infinity;
  }
  const defaultCartTaxon: I_cartShop_bundle_taxon = {
    products: {},
    count: 0,
    taxonId: taxon.id,
    price: taxon.price,
  };
  let [cartTaxon, setCartTaxon] = useState<I_cartShop_bundle_taxon>(
    JSON.parse(JSON.stringify(defaultCartTaxon)),
  );
  ...
  function virayeshe_sabad(
    newCartTaxon: I_cartShop_bundle_taxon,
    delay?: boolean,
  ) {
    ...
    if (!delay) {
      setCart(newCartTaxon);
    } else {
      cartTimeout = setTimeout(() => setCart(newCartTaxon), 1000);
    }
  }
  useEffect(() => {
    try {
  ...
  function virayeshe_taxon_dar_sabad(
    newCartTaxon: I_cartShop_bundle_taxon,
    delay?: boolean,
  ) {
    delay = delay === undefined ? true : delay;
    for (let i = 0; i < taxon.products.length; i++) {
      ...
      if (!cartProduct) {
        newCartTaxon.products[product.id] = { variants: {}, qty, price };
      }
      if (variants.length < 2) {
        let { id, step } = variants[0];
        //در حالت تک واریانتی تعداد واریانت را برابر مقدار نهایی قرار بده
        newCartTaxon.products[product.id].variants[id] = {
          count: qty,
          step,
          variantId: id,
        };
  ...
  function taghire_tedade_baste(dir: 1 | -1) {
    let newCartTaxon: I_cartShop_bundle_taxon = JSON.parse(
      JSON.stringify(cartTaxon),
    );
    newCartTaxon.count += dir;
    if (newCartTaxon.count < 0) {
      newCartTaxon.count = 0;
    }
    if (newCartTaxon.count > taxon.max) {
      newCartTaxon.count = taxon.max;
    }
    if (!newCartTaxon.count) {
      hazfe_taxon_az_sabad();
    } else {
      virayeshe_taxon_dar_sabad(newCartTaxon, false);
    }
  }
  function image_layout() {
    return {
      size: 346,
      className: "theme-box-shadow theme-card-bg theme-border-radius m-h-12",
      column: [
        { size: 24 },
        {
          flex: 1,
          html: <img src={taxon.image} alt="" height="100%" />,
          align: "vh",
        },
        { size: 12 },
        {
          size: 36,
          html: taxon.name,
          className: "fs-14 theme-dark-font-color bold p-h-12",
        },
        {
          size: 36,
          html: "کد کالا : " + taxon.id,
          className: "fs-12 theme-medium-font-color p-h-12",
        },
      ],
    };
  }
  function taxonDetails_layout() {
    return {
      className:
        "theme-card-bg m-h-12 p-12 theme-border-radius theme-box-shadow",
      column: [
        {
          html: `اقلام داخل هر یک بسته`,
          align: "v",
          ...
          column: taxon.products.map(({ qty, name }) => {
            return {
              ...
              row: [
                {
                  align: "vh",
                  html: <img src={bundleBoxSrc as string} alt="" width="30" />,
                },
                {
                  align: "v",
                  html: `تعداد ${qty} عدد ${name}`,
                  className: "theme-medium-font-color fs-12",
                },
              ],
            };
  ...
  function count_layout() {
    return {
      className:
        "theme-card-bg theme-box-shadow theme-border-radius m-h-12 p-12",
      ...
      column: [
        {
          ...
          row: [
            {
              align: "v",
              html: `تعداد بسته را مشخص کنید`,
              className: "theme-dark-font-color fs-14 bold",
            },
            {
              show: taxon.max !== Infinity,
              align: "v",
              html: `( سقف ${taxon.max} بسته )`,
              className: "theme-dark-font-color fs-10",
            },
          ],
        },
        {
          gap: 6,
          row: [
            countButton_layout(1),
            { size: 48, html: cartTaxon.count, align: "vh" },
            countButton_layout(-1),
          ],
        },
      ],
  ...
  function countButton_layout(dir: 1 | -1) {
    ...
    let opacity = 1;
    if (dir > 0 && cartTaxon.count >= taxon.max) {
      opacity = 0.4;
    }
    if (dir < 0 && cartTaxon.count <= 0) {
      opacity = 0.4;
    }
    return {
      size: 40,
      html: <Icon path={path} size={1} />,
      align: "vh",
      onClick: () => taghire_tedade_baste(dir),
      style: {
        background: "#3B55A5",
        height: 40,
        color: "#fff",
  ...
  function productSingleVariant_layout(product: I_bundle_product) {
    return {
      ...
      row: [
        ...
        { size: 6 },
        {
          html: `${product.qty + " واحد"} ${product.name} انتخاب شده`,
          align: "v",
  ...
  function products_layout() {
    ...
    return {
      column: taxon.products.map((product: I_bundle_product) =>
  ...
  function product_layout(product: I_bundle_product) {
    if (product.variants.length < 2) {
      return productSingleVariant_layout(product);
    }
    ...
    return {
      column: [
        ...
        product_layout_message(product, totalCount, fillCount),
        {
          html: (
            <div
              style={{ height: 6, background: "#f8f8f8", width: "100%" }}
            ></div>
          ),
        },
      ],
          ...
          row: [
            {
              size: 30,
              align: "vh",
              html: <Icon path={mdiInformation} size={0.7} />,
              style: { color: "dodgerblue" },
            },
            {
              html: `نوع کالاها در ${cartTaxon.count + " بسته " + product.name} ${`( ${qty * cartTaxon.count} عدد )`} را انتخاب کنید`,
              align: "v",
              className: "theme-dark-font-color fs-12 bold",
            },
          ],
        },
        {
          html: `با حرکت دادن زبانه ها تعداد ${qty * cartTaxon.count} واحد از انواع این محصول را انتخاب کنید`,
          className: "fs-10 bold p-h-24 theme-light-font-color",
        },
      ],
    };
  }
  function product_layout_sliders(
    product: I_bundle_product,
    totalCount: number,
    fillCount: number,
  ) {
    return {
      column: product.variants.map((variant: I_bundle_variant) =>
        slider_layout(product, variant, totalCount, fillCount),
      ),
    };
  }
  function product_layout_message(
    product: I_bundle_product,
    totalCount: number,
    fillCount: number,
  ) {
      ...
      row: [
        {
          size: 30,
          html: (
            <Icon path={isFull ? mdiCheckCircle : mdiAlertCircle} size={0.7} />
          ),
          align: "vh",
        },
        {
          html: `${fillCount + " عدد"} از ${product.qty * cartTaxon.count + " عدد"} کالا تعیین نوع شده`,
          align: "v",
          className: "fs-12 bold",
        },
      ],
  ...
  function slider_layout(
    ...
    return {
      html: <BundleSlider key={product.id + variantId} {...sliderProps} />,
    };
  }
  function footer_layout() {
    return {
      style: { boxShadow: "0 0px 6px 1px rgba(0,0,0,.1)" },
      className: "p-12 bg-fff",
      align: "v",
      row: [
        {
          show: !!cartTaxon.count,
          className: "theme-link-font-color bold",
          html: (
            <button
              disabled={!!submitLoading}
              className="button-2"
              onClick={() => edameye_farayande_kharid()}
            >
              ادامه فرایند خرید
            </button>
          ),
          ...
          column: [
            {
              gap: 6,
              align: "v",
              className: "fs-10 theme-medium-font-color theme-link-font-color",
              row: [
                { flex: 1 },
                { html: "قیمت هر یک بسته", align: "v" },
                {
                  html: `${SplitNumber(taxon.price)}`,
            ...
            },
            {
              ...
              row: [
                { flex: 1 },
                {
                  html: "مجموع",
                  align: "v",
                  className: "fs-10 theme-medium-font-color",
                },
                {
                  html: `${SplitNumber(taxon.price * cartTaxon.count)}`,
                  className: "theme-dark-font-color bold fs-20",
                },
                { html: `ریال`, className: "theme-light-font-color fs-10" },
              ],
            },
  ...
  function fixCart(): boolean {
    ...
    for (let i = 0; i < taxon.products.length; i++) {
      ...
      newCartTaxon.products[product.id].variants[firstVariant.id].count +=
        emptyCount;
      virayeshe_sabad(newCartTaxon);
    }
    return hasError;
  ...
  function edameye_farayande_kharid() {
    ...
    if (hasError) {
      ...
      rsa.addSnakebar({
        text: "تعداد سبد محصولات این بسته به طور اتوماتیک پر شد",
        type: "info",
        onClose: false,
      });
      setTimeout(() => {
  ...
  function body_layout() {
    return {
      flex: 1,
      className: "ofy-auto hide-scroll m-v-12",
      gap: 12,
      column: [image_layout(), taxonDetails_layout(), count_layout()],
    };
  }
  return (
    <RVD
      layout={{
        className: "theme-popup-bg",
        column: [header_layout(), body_layout(), footer_layout()],
      }}
    />
  );
}
...
function BundleSlider(props: I_BundleSlider) {
  ...
  let image;
  if (variantName === "آفتابی") {
    image = aftabisrc;
  } else if (variantName === "مهتابی") {
    image = mahtabisrc;
  } else if (variantName === "یخی") {
    image = yakhisrc;
  }
        ...
        column: [
          {
            html: variantName,
            align: "v",
            className: "theme-medium-font-color fs-12 bold",
          },
          {
            row: [
              {
                ...
                html: (
                  <AIOInput
                    ...
                    scaleStep={[max]}
                    scaleStyle={(value: number) => {
                      if (value === max) {
                        return { background: "dodgerblue" };
                      }
                    }}
                    labelStep={[max]}
                    labelStyle={(value: number) => {
                      if (value === max) {
                        return { color: "dodgerblue", fontSize: 10, top: 43 };
                      }
                    }}
                    start={0}
                    direction="left"
                    ...
                    getPointHTML={
                      !image
                        ...
                        : () => {
                            return (
                              <img src={image as string} width="30" alt="" />
                            );
                          }
                    }
                    lineStyle={{ height: 4 }}
...
function CartButton(props: I_CartButton) {
  ...
  let { shopId, taxonId, shopName } = product.category;
  function openCart(e) {
    e.stopPropagation();
    ...
    if (taxonId) {
      ...
      if (cartTab.taxons[taxonId]) {
        if (cartTab.taxons[taxonId].products[product.id]) {
          if (
            cartTab.taxons[taxonId].products[product.id].variants[variantId]
          ) {
            count = cartTab.taxons[taxonId].products[product.id].variants[
              variantId
            ].count as number;
          }
        }
      ...
      if (cartTab.products[product.id]) {
        if (cartTab.products[product.id].variants[variantId]) {
          count = cartTab.products[product.id].variants[variantId]
            .count as number;
        }
      }
    }
  ...
  if (renderIn === "shipping") {
      ...
      html: () => (
        <button
          onClick={async () => {
            ...
            // پیدا کردن متغیر با استفاده از variantId
            let variant = product.variants.find(v => v.id === variantId);

            // بررسی اینکه آیا متغیر یافت شده است یا خیر
            if (variant) {
              // استخراج قیمت از متغیر
              ...
              window.dataLayer.push({
                ...
                ecommerce: {
                  items: {
                    ...
                    variant: variantId,
                    productCategory: product.category.taxonName,
                    campaign: product.category.shopName,
                    count: 1
                  }
                }
              });
            ...
            //console.log(`Added 1 of ${product.name} at ${product.Price} each to the cart.`);
    ...
    layout = {
      column: [
        {
          show: renderIn === "product",
          align: "vh",
          html: "تعداد در سبد خرید",
          className: "fs-12 color3B55A5",
        },
        {
          html:  () => (
...
function ProductCount(props: I_ProductCount) {
  ...
  useEffect(() => {
      ...
      try {
        if (Shop[shopId]?.CampaignId === 63) {
          ...
          for (let i = 0; i < MarketingLines.length; i++) {
            let { CampaignDetails = {}, ItemCode, ItemQty } = MarketingLines[i];
            let { Information, BundleRowsInfos = {}, RequestedQty, Status } = CampaignDetails;
            let { minQty, maxQty, qty, maxQtyCanOrder } = BundleRowsInfos;
            setMin(minQty); 
            setMax(maxQty);  
            setReq(RequestedQty); 
            setQTY(qty); 
            setStatus(Status); 
            setMaxCanOrder(maxQtyCanOrder)
          }
        }
      } catch (error) {
        console.error("Error fetching data:", error);
    ...
    fetchData();
  }, [Shop, shopId, actionClass, props.value]);
  ...
  // else if ( value < Max && status < 0) {
  //   value = req
  // }

  // else if ( (value + maxCanOrder) < max && status === 2) {
  //   value = maxCanOrder
  // }
        ...
        row: [
          {
            html: (
              <div
                ...
                onTouchStart={(e) => touchStart(0, touch, true)}
                className={
                  "product-count-button" + (value >= max ? " disabled" : "")
                }
              >
          ...
          },
          {
            ...
            html: <div className="product-count-input">{value}</div>,
          },
          {
            html: () => (
              <div
              ...
              row: [
                {
                  flex: 1,
                  html: (
                    <button className="button-1" onClick={() => onRemove()}>
                      حذف محصول
                    </button>
                  ),
                },
...
export function Cart(props: I_Cart) {
  ...
  async function update(id = activeTabId) {
    if (id !== false && !cart.shops[id]) {
      id = false;
    }
    let keys = Object.keys(cart.shops);
    if (id === false && keys.length) {
      id = keys[0];
    }
    setActiveTabId(id);
    if (id) {
      ...
      setFactor(factor);
      //console.log(items)
      // Group items by product name and variant ID
      let groupedItems = {};
      items.forEach(item => {
        let variantId = item.props.product.variants[0]?.id; // استخراج variantId
        let key = item.props.product.name + '-' + variantId;
        if (!groupedItems[key]) {
          groupedItems[key] = {
            name: item.props.product.name,
            id: variantId,
            price: item.props.product.Price,
            productCategory: item.props.product.category.taxonName,
            campaign: item.props.product.category.shopName,
          };
        }
      });

      let dataLayerProducts = Object.values(groupedItems);

      window.dataLayer = window.dataLayer || [];
      window.dataLayer.push({
        event: 'page_view',
        page_title: 'سبد خرید',
        page_url: '/burux_shop/cart',
        ecommerce: {
          items: dataLayerProducts
        }
      });

      //console.log("Cart viewed:", dataLayerProducts);
    } else {
      setItems([]);
      setFactor(false);
    }
  ...
  function getBadge(option: string) {
    ...
    if (cartTab.type === "Taxon") {
      length = Object.keys(cartTab.taxons).length;
    } else if (cartTab.type === "Bundle") {
      length = Object.keys(cartTab.taxons).length;
    } else {
      length = Object.keys(cartTab.products).length;
    }
  ...
  function empty_layout() {
    return {
      ...
      column: [
        {
          html: (
            <img src={noItemSrc as string} alt="" width="128" height="128" />
          ),
        },
  ...
  function products_layout() {
    ...
    return {
      flex: 1,
      className: "ofy-auto hide-scroll",
      column: items.map((cartItem) => {
        return { html: cartItem, className: "of-visible hide-scroll" };
      }),
    };
  }
  function payment_layout() {
...
type I_Shipping = { shopId: string; shopName: string; count?: number };
function Shipping(props: I_Shipping) {
  let {
    ...
    actionClass,
    msfReport,
  }: I_app_state = useContext(appContext);
  let { shopId, shopName, count } = props;
  let [code, setCode] = useState<{ discount: string; gift: string }>({
    discount: "",
    gift: "",
  });
  ...
  let [codeInfo, setCodeInfo] = useState<{ discount: any; gift: any }>({
    discount: undefined,
    gift: undefined,
  });
  let [data, setData] = useState<{
    [key: string]: { value: number; options: any[] };
  }>();
  let [mounted, setMounted] = useState<boolean>(false);
  let [factorItems, setFactorItems] = useState<I_factorItem[]>([]);
  let [cartItems, setCartItems] = useState<any[]>([]);
  function details_layout(
    list: { key: string; value: any; className?: string }[],
  ) {
    return {
      className: "box p-12 m-h-12",
      column: list.map(
        ({ key, value, className = "theme-medium-font-color fs-14" }) => {
          return {
            size: 36,
            childsProps: { align: "v" },
            row: [
  ...
  function getParams(data) {
    ...
    return {
      SettleType,
      PayDueDate: PayDueDate.value,
      DeliveryType: DeliveryType.value,
      PaymentTime: PaymentTime.value,
      address: userInfo.address,
      giftCodeInfo: codeInfo.gift,
      discountCodeInfo: codeInfo.discount,
    };
  }
  ...
  async function onMount() {
    let backOfficeData =
      backOffice[shopId] ||
      backOffice.spreeCampaigns.find((o) => o.shopId === shopId);
    let {
      PayDueDate,
      PaymentTime,
      DeliveryType,
    ...
    if (
      Array.isArray(Shop[shopId].PayDueDates) &&
      Shop[shopId].PayDueDates.length
    ) {
    ...
    let data = {
      PayDueDate: {
        value: PayDueDate,
        options: backOffice.PayDueDate_options.filter(
          ({ value }) => PayDueDates.indexOf(value) !== -1,
        ),
      },
      PaymentTime: {
        value: PaymentTime,
        options: backOffice.PaymentTime_options.filter(
          ({ value }) => PaymentTimes.indexOf(value) !== -1,
        ),
      },
      DeliveryType: {
        value: DeliveryType,
        options: backOffice.DeliveryType_options.filter(
          ({ value }) => DeliveryTypes.indexOf(value) !== -1,
        ),
      },
    };
    setData(data);
  ...
  function address_layout() {
    return {
      ...
      column: [
        ...
        {
          className: "fs-14 theme-medium-font-color bgF1F1F1 p-12 br-4",
          html: userInfo.address,
          size: 72,
        },
      ],
    };
      ...
      column: [
        {
          size: 36,
          align: "v",
          className: "theme-medium-font-color fs-14 bold p-h-12",
          html: "محصولات",
        },
        {
          className: "of-visible hide-scroll",
          column: cartItems.map((card) => {
            return { html: card, className: "of-visible hide-scroll" };
          }),
        },
        { size: 12 },
      ],
  ...
  function codeView_layout() {
    ...
    if (codeInfo.gift) {
      try {
        let details = actionClass.getCodeDetails({
          giftCodeInfo: codeInfo.gift,
          discountCodeInfo: codeInfo.discount,
        });
        if (details.PromotionValue) {
          column.push({
            style: { color: "green" },
            html: `کارت هدیه به مبلغ ${details.PromotionValue} روی فاکتور اعمال شد`,
          });
        }
      } catch {}
    }
    if (codeInfo.discount) {
      try {
        ...
        if (details.DiscountPercentage) {
          column.push({
            style: { color: "green" },
            html: `کد تخفیف ${`${details.DiscountPercentage} درصد`} تا سقف ${details.DiscountMaxValue} روی فاکتور اعمال شد `,
          });
        }
      } catch {}
    }
  ...
  function amount_layout() {
    ...
    let paymentValue,deliveryValue;
    //PaymentTime => زمان پرداخت
    if(data.PaymentTime.value === 5){paymentValue = 'اینترنتی'}
    else if(data.PaymentTime.value === 1){paymentValue = 'واریز قبل ارسال'}
    else if(data.PaymentTime.value === 2){paymentValue = 'واریز پای بار'}
    //DeliveryType => نحوه ارسال
    if(data.DeliveryType.value === 11){deliveryValue = 'ماشین توزیع بروکس'}
    else if(data.DeliveryType.value === 12){deliveryValue = 'ماشین اجاره ای'}
    else if(data.DeliveryType.value === 13){deliveryValue = 'باربری'}
    else if(data.DeliveryType.value === 14){deliveryValue = 'پخش گرم'}
    else if(data.DeliveryType.value === 15){deliveryValue = 'ارسال توسط ویزیتور'}

    const productItems = cartItems.map(item => ({name: item.props.product.name, price: item.props.product.FinalPrice}));
    const factorDetails = factorItems.map(item => ({ key: item.key, value: item.value }));


    if (data.PaymentTime.value === 5) {
      return {
        ...
        column: [
          ...
          { size: 6 },
          {
            size: 36,
            align: "vh",
            className: "theme-medium-font-color fs-14 bold",
            html: (
              <button
                className="button-2"
                onClick={async () => {
                  ...
                  await apis.request({
                    api: "kharid.pardakht",
                    description: "عملیات ثبت و پرداخت",
                    parameter,
                    onSuccess: () => {
                      msfReport({
                        actionName: "send to visitor",
                        actionId: 234,
                        tagName: "kharid",
                        result: "success",
                        eventName: "action",
                      ...
                      pushToDataLayer({
                        event: 'page_view',
                        page_title: 'اطلاعات پرداخت و ارسال',
                        page_url: '/burux_shop/checkout/online',
                        ecommerce: {
                          items: {
                            userAddress: parameter.address,
                            user: `${userInfo.firstName} ${userInfo.lastName}`,
                            phoneNumber: userInfo.phoneNumber,
                            userGroup: b1Info.customer.groupName,
                            userCode: userInfo.cardCode,
                            PaymentTime: paymentValue,
                            DeliveryType: deliveryValue,
                            campaign: shopName,
                            products: productItems,
                            factorItems: factorDetails,
                          }
                        }
                      });
                    },
                  ...
                  await apis.request({
                    ...
                    onSuccess: () => {
                      ...
                      pushToDataLayer({
                        event: 'page_view',
                        page_title: 'اطلاعات پرداخت و ارسال',
                        page_url: '/burux_shop/checkout/visitor',
                        ecommerce: {
                          items: {
                            userAddress: parameter.address,
                            user: `${userInfo.firstName} ${userInfo.lastName}`,
  ...
  function getCodeAfter(type: "gift" | "discount") {
    ...
    return (
      <button
        ...
        onClick={() => checkCode(type)}
        className={"shipping-code-button" + (className ? " " + className : "")}
      >
        {text}
      </button>
    );
  ...
  function code_layout(type: "gift" | "discount") {
    ...
    let value = code[type];
    return {
      className: "box m-h-12 p-r-12 m-b-12",
      html: (
        <AIOInput
          key={type}
          type="text"
          value={value}
          disabled={!!codeState[type]}
          style={{ height: 36 }}
          placeholder={`${placeholder} را وارد کنید`}
          onChange={(v) => setCode({ ...code, [type]: v })}
          after={getCodeAfter(type)}
          before={placeholder}
        />
      ),
    };
  ...
  function options_layout(key, title, cond = true) {
    ...
    if (
      key === "PaymentTime" &&
      b1Info.customer.purchaseState &&
      b1Info.customer.purchaseState.onlyByOrder
    ) {
    ...
    if (
      key === "PayDueDate" &&
      b1Info.customer.purchaseState &&
      b1Info.customer.purchaseState.onlyByCash
    ) {
        ...
        },
        {
          html: (
            <AIOInput
              ...
              type="radio"
              // options={
              //   shopName === "ده سالگی بروکس" && key === "PaymentTime"
              //     ? (options = [{ value: 5, text: "اینترنتی" }])
              //     : options
              // }
              options={options}
              value={value}
              optionAfter={(option) => {
                ...
                else if (option.text === "واریز قبل ارسال" && shopName === "ده سالگی بروکس") {
                  return (
                    <div style={{ whiteSpace: "nowrap" }}>
                      {"22% تخفیف"}
                    </div>
                ...
                else if (option.text === "واریز قبل ارسال") {
                    return (
                      <div style={{ whiteSpace: "nowrap" }}>
                        {"2% تخفیف مازاد"}
                      </div>
                ...
                else if (option.discountPercent) {
                  return (
                    <div style={{ whiteSpace: "nowrap" }}>
  ...
  //اضافه کردم برای هندل کردن درصد تخفیف زمان پرداخت
  function option_layout(key, title, cond = true) {
    if (!data) {
      return false;
    }
    //console.log('data:',data)
            ...
            //   }
              ...
              optionAfter={(option) => {
                ...
                if (option.discountPercent) {
                  return (
                    <div style={{ whiteSpace: "nowrap" }}>
                      {`${option.discountPercent + 2}% تخفیف`}
                    </div>
                  );
                }
                return null;
          ...
          column: [
            {
              ...
              column: [
                ...
                options_layout(
                  "PayDueDate",
                  "مهلت تسویه",
                  data.PaymentTime.value !== 5 && data.PaymentTime.value !== 1,
                ),
                option_layout(
                  "PayDueDate",
                  "مهلت تسویه",
                  data.PaymentTime.value !== 5 && data.PaymentTime.value !== 2,
                ),
                //{ size: 12 },
                // code_layout('gift'),      
                codeView_layout(),
...
export function CategorySlider(props: I_CategorySlider) {
  ...
  function products_layout() {
    ...
    return {
      gap: 16,
      className: "ofx-auto p-b-12",
      row: products.map((product, i) => {
        return {
          className: "of-visible hide-scroll",
          html: Shop.Regular.renderCard_Regular({
            type: "vertical",
            product,
            loading,
            renderIn: "slider",
        ...
        column: [
          {
            ...
            row: [
              ...
              { flex: 1 },
              {
                show: !!showAll && !!products && !!products.length,
                html: "مشاهده همه",
                className: "fs-12 theme-link-font-color bold",
                align: "v",
                onClick: () => showAll(),
              },
            ],
